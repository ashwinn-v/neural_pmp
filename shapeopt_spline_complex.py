...
class BSplineEnv(ContinuousEnv):
    ...
    def sample_q(self, num_examples, mode='train'):
        """
        Sample random control points around a *star-like* shape
        rather than a simple circle. You can adjust the baseline 
        shape however you want for complexity.
        """
        # For example, let's define a 'star' shape in polar coordinates:
        # r(θ) = 1 + 0.5*cos(5θ) to get 5 'lobes'.
        # We'll generate 'n_coeff' points along one full rotation.
        # Then we add random noise to break exact symmetry.
        
        angles = np.linspace(0, 2*np.pi, self.n_coeff, endpoint=False)
        
        # Star shape radius:  r(θ) = 1 + 0.5 cos(5θ)
        # Feel free to tweak the amplitude (0.5) and frequency (5) for more complexity
        r_star = 1.0 + 0.5 * np.cos(5 * angles)
        
        # Baseline star shape
        base_x = r_star * np.cos(angles)
        base_y = r_star * np.sin(angles)
        
        # Adjust noise scale by mode if desired
        if mode == 'train':
            noise_scale = 0.1
        else:
            noise_scale = 0.05
